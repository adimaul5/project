var save_method;
var link = "<?php echo site_url('M_gudang')?>";
var table;
...
function reset() {
		//document.getElementById("fc_kdgudang"+mdl).value = '';
		document.getElementById("fv_nmgudang"+mdl).value = '';
...
function save() {
			...
			var url;
			if (save_method == 'add') {
				url = linke+"M_gudang/ajax_add";
			} else {
				url = linke+"M_gudang/ajax_update"; 
			}
...
function edit(id) {
			...
			$.ajax({
				url : linke+"M_gudang/ajax_edit/"+id,
				type: "GET",
				dataType: "JSON",
				success: function(result) {  
...
function hapus(id) {
		if (confirm('Are you sure delete this data?')) {
			$.ajax ({
				url : linke+"M_gudang/ajax_delete/"+id,
				type: "POST",
				dataType: "JSON",
