var save_method;
var link = "<?php echo site_url('M_jenis_telur')?>";
var table;
...
function lihatData(){
	    ...
	    $("#tabelData"+mdl).fadeOut("slow", function(){
			...
			$.ajax({
				type: "POST",
				url: linke+"M_jenis_telur/act_table",
...
function save() {
			...
			var url;
			if (save_method == 'add') {
				url = linke+"M_jenis_telur/ajax_add";
			} else {
				url = linke+"M_jenis_telur/ajax_update"; 
			}
...
function edit(id) {
			...
			$.ajax({
				url : linke+"M_jenis_telur/ajax_edit/"+id,
				type: "GET",
				dataType: "JSON",
				success: function(result) {  
					//document.getElementById('fc_kdbahan').setAttribute('readonly','readonly');
					$('[name="fc_id"]').val(result.fc_id);
					$('[name="fv_nama_kandang"]').val(result.fc_kdjenis_telur);
					$('[name="fc_jumlah_ayam"]').val(result.fv_jenis_telur);
...
function hapus(id) {
		if (confirm('Are you sure delete this data?')) {
			$.ajax ({
				url : linke+"M_jenis_telur/ajax_delete/"+id,
				type: "POST",
				dataType: "JSON",
