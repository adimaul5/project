...
function reset() {
		document.getElementById("fc_nopol"+mdl).value = '';
		document.getElementById("fv_ket"+mdl).value = '';
		document.getElementById("fv_nama_sopir"+mdl).value = '';
...
function save() {
			...
			var url;
			if (save_method == 'add') {
				url = linke+"M_armada/ajax_add";
			} else {
				url = linke+"M_armada/ajax_update"; 
			}
...
function edit(id) {
			...
			$.ajax({
				url : linke+"M_armada/ajax_edit/"+id,
				type: "GET",
				dataType: "JSON",
				success: function(result) {  
					//document.getElementById('fc_kdbahan').setAttribute('readonly','readonly');
					$('[name="fc_id"]').val(result.fc_id);
					$('[name="fc_nopol"]').val(result.fc_nopol);
					$('[name="fv_ket"]').val(result.fv_ket);
					$('[name="fv_nama_sopir"]').val(result.fv_nama_sopir);;
...
function hapus(id) {
		if (confirm('Are you sure delete this data?')) {
			$.ajax ({
				url : linke+"M_armada/ajax_delete/"+id,
				type: "POST",
				dataType: "JSON",
