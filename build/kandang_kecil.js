var save_method;
var link = "<?php echo site_url('M_kandang_kecil')?>";
var table;
...
function lihatData(){
	    ...
	    $("#tabelData"+mdl).fadeOut("slow", function(){
			...
			$.ajax({
				type: "POST",
				url: linke+"M_kandang_kecil/act_table",
...
function save() {
			...
			var url;
			if (save_method == 'add') {
				url = linke+"M_kandang_kecil/ajax_add";
			} else {
				url = linke+"M_kandang_kecil/ajax_update"; 
			}
...
function edit(id) {
			...
			$.ajax({
				url : linke+"M_kandang_kecil/ajax_edit/"+id,
				type: "GET",
				dataType: "JSON",
				success: function(result) {  
					//document.getElementById('fc_kdbahan').setAttribute('readonly','readonly');
					$('[name="fc_kdkandang"]').val(result.fc_kdkandang_kecil);
					$('[name="fv_nama_kandang"]').val(result.fv_nama_kandang_kecil);
...
function hapus(id) {
		if (confirm('Are you sure delete this data?')) {
			$.ajax ({
				url : linke+"M_kandang_kecil/ajax_delete/"+id,
				type: "POST",
				dataType: "JSON",
